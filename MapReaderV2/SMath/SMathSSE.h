...
*
* @param X		1st ULONG component
* @param Y		2nd ULONG component
* @param Z		3rd ULONG component
* @param W		4th ULONG component
* @return		Bitwise equivalent vector with 4 floats
*/
FORCEINLINE VectorRegister MakeVectorRegister(ULONG X, ULONG Y, ULONG Z, ULONG W)
{
	union { VectorRegister v; ULONG f[4]; } Tmp;
	Tmp.f[0] = X;
	Tmp.f[1] = Y;
	Tmp.f[2] = Z;
	Tmp.f[3] = W;
...
static const VectorRegister SSE_XYZ_MASK = MakeVectorRegister((ULONG)0xffffffff, (ULONG)0xffffffff, (ULONG)0xffffffff, (ULONG)0x00000000);

/** Bitmask to AND out the sign bit of each components in a vector */
#define SIGN_BIT (~(1 << 31))
static const VectorRegister SSE_SIGN_MASK = MakeVectorRegister((ULONG)SIGN_BIT, (ULONG)SIGN_BIT, (ULONG)SIGN_BIT, (ULONG)SIGN_BIT);
...
#if __GNUC__
	*((ULONG*)Ptr) = (ULONG)_mm_cvtsi64_si32(NewVec);
#else
	*((ULONG*)Ptr) = NewVec.m64_u32[0];
#endif
}
...
*
* @return			The ULONG control register
*/
...
*
* @param ControlStatus		The ULONG control status value to set
*/
...
inline BOOL VectorContainsNaNOrInfinite(const VectorRegister& Vec)
{
	...
	BOOL IsNotNAN = move_mask(_mm_cmpeq_ps(Vec, Vec)); // Test for the fact that NAN != NAN
