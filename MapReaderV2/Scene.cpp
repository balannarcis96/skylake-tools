...
bool Scene::GetAllNavmeshObjects(TRef<Terrain>& outTerrain, std::vector<StaticMeshActor*>& actors)
{
	auto SelectedPackage = GGUIManager.GetElementAs<PackagesTool>()->GetSelectedPackage();
	if (!SelectedPackage.Get()) {
		return false;
	}

	if(SelectedPackage->levels[0]->terrains.size()) {
		//@TODO check for multiple levels or terrain
		outTerrain = SelectedPackage->levels[0]->terrains[0];
	}

	for (auto* Pkg : AllPackages) {
		for (auto* actor : Pkg->actorsMeshees)
		{
			D3DXVECTOR3 min = { FLT_MAX,FLT_MAX ,FLT_MAX }, max{ FLT_MIN,FLT_MIN ,FLT_MIN };

			actor->GetMinAndMax(min, max);

			if (AABB.Box.Intersect(SBox({ min.x,min.y,min.z }, { max.x,max.y,max.z }))) {
...
void Scene::RenderPackages(ID3D11DeviceContext* Context)
{
	auto Shader = GD3DDriver.GetShaderPack(AppShader::MainShaderPack);
	if (Shader.Get()) {
	...
	{
		if (!AllPackages[i]->Visible) {
			continue;
		}

		for (size_t j = 0; j < AllPackages[i]->levels.size(); j++)
		{
			AllPackages[i]->levels[j]->Render();
		}
	}
}
...
void Scene::Render(float dt)
{
	...
	RenderPackages(Context);

	GGUIManager.GetElementAs<NavmeshTool>()->Render(Context, dt);
...
void Scene::RenderUI()
{
	...
	if (ImGui::BeginTabBar("Tabs"))
	{
		...
		{
			uint32 fps = (uint32)(1.0f / GApp.GetTimer().Elapsed);
...
void SceneAABB::FromPackageLevel(UPackage* Package) noexcept
{
	Box.Min.X = FLT_MAX;
	Box.Min.Y = FLT_MAX;
	Box.Min.Z = FLT_MAX;
	...
	for (size_t j = 0; j < Package->levels.size(); j++)
	{
		for (auto* Terrain : Package->levels[j]->terrains) {

			Box.Min.X = Min(Box.Min.X, Terrain->minVertex.x);
			Box.Min.Y = Min(Box.Min.Y, Terrain->minVertex.y);
			Box.Min.Z = Min(Box.Min.Z, Terrain->minVertex.z);

			Box.Max.X = Max(Box.Max.X, Terrain->maxVertex.x);
			Box.Max.Z = Max(Box.Max.Z, Terrain->maxVertex.z);
		}
	}

	auto* Vertices = GetVertices();

	Vertices[0].position.x = Box.Min.X;
	Vertices[0].position.y = Box.Min.Y;
	Vertices[0].position.z = Box.Min.Z;

	Vertices[1].position.x = Box.Max.X;
	Vertices[1].position.y = Box.Min.Y;
	Vertices[1].position.z = Box.Min.Z;

	Vertices[2].position.x = Box.Min.X;
	Vertices[2].position.y = Box.Max.Y;
	Vertices[2].position.z = Box.Min.Z;

	Vertices[3].position.x = Box.Max.X;
	Vertices[3].position.y = Box.Max.Y;
	Vertices[3].position.z = Box.Min.Z;

	Vertices[4].position.x = Box.Min.X;
	Vertices[4].position.y = Box.Min.Y;
	Vertices[4].position.z = Box.Max.Z;

	Vertices[5].position.x = Box.Max.X;
	Vertices[5].position.y = Box.Min.Y;
	Vertices[5].position.z = Box.Max.Z;

	Vertices[6].position.x = Box.Min.X;
	Vertices[6].position.y = Box.Max.Y;
	Vertices[6].position.z = Box.Max.Z;

	Vertices[7].position.x = Box.Max.X;
	Vertices[7].position.y = Box.Max.Y;
	Vertices[7].position.z = Box.Max.Z;
