...
private:
	...
	D3DXVECTOR3				Position = { 0.0f, 100.0f, 0.0f };
...
struct SceneAABB :SceneMesh<D3DColorVertex, UINT16, true> {
	...
	void FromPackageLevel(UPackage* Package) noexcept;
...
struct Scene : UIElement<UIElementType::Scene> {
	...
	bool ContainsPackage(const char* Name) {
		return std::find_if(AllPackages.begin(), AllPackages.end(), [&](UPackage* Package) -> bool {
			return !strcmp(Package->PackageNameWithExtension.c_str(), Name);
			}) != AllPackages.end();
	}
	const std::vector<UPackage*>& GetLoadedPackages() const noexcept {
		return AllPackages;
	}
	...
	void OnPackageSelected(UPackage* Package) {
		if (!Package) {
		...
		SetAABBVisible(true);
		AABB.FromPackageLevel(Package);
	}

	D3DXVECTOR4					ClearColor = CLEAR_COLOR;
	SceneAABB					AABB;

	bool GetAllNavmeshObjects(TRef<Terrain>& outTerrain, std::vector<StaticMeshActor*>& actors);

private:
	void RenderPackages(ID3D11DeviceContext* Context);
	void Begin();
	void End();

	BOOL						IsValid = FALSE;
	Camera						Camera;
	std::vector<UPackage*>		AllPackages;
	SceneGrid					SceneGrid;
	SceneLight					Light;
};
